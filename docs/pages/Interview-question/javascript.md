...
1. call、apply、bind都可以改变函数内部this指向。
2. call 和 apply 会调用函数，bind 不会调用函数。
3. call 和 apply 传递的参数不一样，call 传递参数arg1,arg2...形式 apply 必须数组形式 [arg]。
...
4. for of：可以遍历、数组、部分类数组、字符串、Set和Map，如果对象默认不具有遍历器机制，如果想要使用for of遍历就必须在对象原型上加遍历器。
...
1. let和const声明的变量不会提升，如果块区中存在 let 和 const 声明，就会形成封闭作用域不允许重复声明。
2. 如果在let和const声明之前使用变量会报错。
3. let定义的变量可以被修改，const定义的是常量，不能修改，但是如果const定义的是对象，可以修改对象内部的数据。
...
1. 作用域链可以理解为一组对象列表，包含父级和自身的变量对象，因此我们便能通过作用域链访问到父级里声明的变量或者函数。
